...
export default async function authRoutes(fastify) {
    ...
    fastify.post('/signup/send-otp', async (request, reply) => {
        ...
        if (hourlyCount > 3)
            return reply.code(429).send({ error: 'RATE_LIMITED', retry_after_seconds: 3600 });
    ...
    fastify.post('/refresh', async (request, reply) => {
        ...
        if (!oldToken)
            return reply.code(401).send({ error: 'REFRESH_TOKEN_NOT_FOUND' });
        if (oldToken.revoked)
            return reply.code(401).send({ error: 'TOKEN_THEFT_DETECTED' });
